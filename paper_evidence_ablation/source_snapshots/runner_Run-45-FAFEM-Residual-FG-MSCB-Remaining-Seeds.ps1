param(
    [ValidateSet(24)][int] $BatchSize = 24,
    [switch] $DryRun
)

$ErrorActionPreference = "Stop"
$experiment = "one_seed_45_fafem_residual_frequency_guided_mscb_stage3_stronger_init_warmup_cosine"
$outputName = "45_fafem_residual_frequency_guided_mscb_stage3_stronger_init_warmup_cosine"
$remainingSeeds = @(6543, 7777)
$repoRoot = Split-Path -Parent $PSScriptRoot
$projectRoot = Split-Path -Parent $repoRoot
$python = Join-Path $projectRoot ".venv\Scripts\python.exe"
if (-not (Test-Path -LiteralPath $python -PathType Leaf)) { $python = Join-Path $repoRoot ".venv\Scripts\python.exe" }
if (-not (Test-Path -LiteralPath $python -PathType Leaf)) { $python = "python" }
. (Join-Path $PSScriptRoot "Training-RunnerLease.ps1")

function Get-SeedState([int] $Seed) {
    $seedDir = Join-Path $repoRoot "one_seed_results\ablation\$outputName\seed_$Seed"
    $result = & $python (Join-Path $repoRoot "ablation_state.py") --experiment_name $experiment --seed $Seed --seed_dir $seedDir --max_epochs 200 --log_path (Join-Path $seedDir "KvasirSEG-ConvNeXt_log.txt")
    if ($LASTEXITCODE -ne 0) { throw "State inspection failed for seed $Seed." }
    return $result | ConvertFrom-Json
}

if (-not $DryRun) {
    $activeWork = Get-CimInstance Win32_Process | Where-Object { $_.Name -match '^python(\.exe)?$' -and $_.CommandLine -match '(?i)(main_torch\.py|evaluate\.py)' }
    if ($activeWork) { throw "Another training or evaluation process is active (PID: $($activeWork.ProcessId -join ', '))." }
}

$lease = $null
if (-not $DryRun) { $lease = Enter-TrainingRunnerLease -LeasePath (Join-Path $repoRoot ".one_seed_ablation_training.lock") }
try {
    $dryRunPayload = @()
    foreach ($seed in $remainingSeeds) {
        if (-not $DryRun) {
            $state = Get-SeedState $seed
            if ($state.training_action -eq "skip" -and $state.evaluation_valid) {
                Write-Host "[$outputName][seed $seed] Valid training and evaluation found - skipping."
                continue
            }
        }
        $invokeSeed = {
            param([int] $TargetSeed, [bool] $IsDryRun)
            & (Join-Path $PSScriptRoot "Invoke-OneSeed-Ablation.ps1") `
            -Experiment $experiment -OutputName $outputName -SkipMode normal `
            -DeepSupervisionHeads 0 -BatchSize $BatchSize -Seed $TargetSeed -Epochs 200 `
            -DecoderWarmupEpochs 0 -UnfreezeSchedule none -LrScheduler warmup_cosine `
            -LrWarmupEpochs 5 -LearningRate 3e-4 -MinimumLearningRate 1e-6 `
            -OptimizerProfile layerwise_convnext -EncoderLayerDecay 0.8 `
            -WeightDecay 1e-4 -EncoderWeightDecay 5e-2 -NewLayerWeightDecay 1e-2 `
            -MaxGradNorm 1.0 -EnableCSAF $false -EnableFAFEM $true `
            -FAFEMStage1 $false -FAFEMStage2 $false -FAFEMStage3 $false `
            -EnableMSC $false -EnableUGBR $false -EnableGatedSkipStage3 $false `
            -EnableCrossLevelFusion $false -EnableGeometryConvStage3 $false `
            -EnableMSCBLiteStage3 $false -EnableFGMSCBLiteStage3 $false `
            -EnableResidualFGMSCBLiteStage3 $true -ResidualFGMSCBGuidanceInitStd 0.0 `
            -ResidualFGMSCBSignedStrength $false -ResidualFGMSCBInitialStrength 0.05 `
            -EnableDeformableResidualFGMSCBLiteStage3 $false `
            -EnablePartialDeformableResidualFGMSCBLiteStage3 $false `
            -EnableResidualFGMSCBAllSkips $false -EnableF4F3ContextGuidedMSCBLiteStage3 $false `
            -EnableMSCBLiteStage2 $false -EnableMSCBLiteStage1 $false `
            -EnableLKALiteStage3 $false -EnableFrequencyAugmentation $false `
            -UncertaintyRefinementVersion none -DryRun:$IsDryRun
        }
        if ($DryRun) { $dryResult = & $invokeSeed $seed $true }
        else {
            & $invokeSeed $seed $false
        }
        if ($LASTEXITCODE -ne 0) { throw "Runner failed for seed $seed with exit code $LASTEXITCODE." }
        if ($DryRun) { $dryRunPayload += ($dryResult | ConvertFrom-Json) }
    }
    if ($DryRun) { $dryRunPayload | ConvertTo-Json -Compress -Depth 10 }
} finally {
    if ($null -ne $lease) { Exit-TrainingRunnerLease -Lease $lease }
}
