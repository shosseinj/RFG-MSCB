...
convolutional backbone with a lightweight decoder and demonstrates that strong
...
backbone with strong representation capacity \cite{convnext}. In this work,
ConvNeXt-Tiny is used as the encoder, while the novelty is concentrated in the
decoder interaction between bottleneck frequency information and Stage-3
...
architectures can introduce additional architectural complexity. The present
model therefore keeps a convolutional encoder--decoder form and asks a narrower
question: whether a compact bottleneck frequency descriptor can guide the
...
MSCB is prior work and is not introduced in this paper. The limitation relevant
to the present method is that a conventional MSCB aggregation treats the branch
...
The Frequency-Aware Feature Enhancement Module (FAFEM) is also treated as prior
work in this paper. It is adopted at the bottleneck to enhance deep features and
produce a low-/high-frequency descriptor. The contribution of this work is not
FAFEM itself, but the reuse of its descriptor as cross-level guidance for
multi-scale convolution in the decoder. The exact original FAFEM citation should
be inserted once the source title, DOI, arXiv identifier, or publisher record is
verified.
...
The proposed architecture does not add an explicit boundary branch, edge label,
or boundary loss. Boundary accuracy is instead expected to benefit indirectly
from adaptive multi-scale decoding and from the use of frequency-guided branch
weights at the first decoder fusion stage. Consequently, claims about boundary
improvement should be supported by the experimental results rather than by
describing a dedicated boundary module.
...
refinement. The proposed method is positioned at the intersection of efficient
...
a lightweight residual bottleneck, Stage-3 SimpleFusion, the proposed
Residual Frequency-Guided MSCB, and standard remaining decoder stages. FAFEM and
MSCB are reused prior modules. The proposed contribution is the coupling between
them: the bottleneck FAFEM frequency descriptor adaptively weights MSCB branches
...
cross-level connection. This framing separates the new RFG-MSCB mechanism from
the prior components on which it is built.
